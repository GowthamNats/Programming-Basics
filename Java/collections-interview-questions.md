...
## Q. If you store Employee object as key say: Employee emp = new Employee(“name1”,20); store it in a HashMap as key, now if we add a new parameter emp.setMarriedStatus(true) and try to override it what will happen?
 ...
 In Java, if you use an object as a key in a HashMap, the key is actually the memory address (hash code) of the object, not the object itself. When you add an object as a key to a HashMap, the HashMap internally calculates the hash code of the object and uses it to determine the bucket where the key-value pair will be stored.

Now, if you modify the state of the object after it has been used as a key in the HashMap, it can lead to unexpected behavior. The hash code of the object is calculated based on its state at the time it was added to the HashMap. If you change the state of the object, its hash code will be different, and the HashMap will not be able to locate the corresponding bucket to access the associated value correctly.
...
public class Employee {
    ...
    @Override
    public int hashCode() {
        // A simple hash code implementation for demonstration purposes
    ...
    public static void main(String[] args) {
        ...
        Employee emp = new Employee("name1", 20);
        employeeMap.put(emp, "Employee 1");

        System.out.println("Value associated with emp: " + employeeMap.get(emp)); // Output: Employee 1

        // Modify the state of the object
        emp.setAge(25);

        System.out.println("Value associated with emp: " + employeeMap.get(emp)); // Output: null
    }
}
```

In this example, we have an `Employee` class with a `hashCode` method that calculates the hash code based on the name and age of the employee. We add an `Employee` object (`emp`) as a key to the `employeeMap`, and the value associated with it is "Employee 1".

After that, we modify the `age` of the `emp` object using the `setAge` method. As a result, the hash code of the `emp` object changes because the hash code calculation depends on both the name and the age. When we try to retrieve the value associated with `emp` from the `employeeMap`, it returns `null` because the modified `emp` object is no longer located in the same bucket where it was originally stored in the `HashMap`.

To avoid such issues, it's generally recommended not to use mutable objects as keys in a HashMap. If you need to modify an object that is used as a key in a HashMap, it's better to remove the key-value pair from the HashMap, modify the object, and then add it back with the updated state.
...
The reason why the `Map` interface does not extend the `Collection` interface in Java is because they serve fundamentally different purposes and have distinct behaviors.

1. Purpose and Data Structure:
   - `Collection`: The `Collection` interface is designed to represent a group of individual elements, where each element is independent and has no specific association with other elements in the collection. Collections are typically used to store and manage a group of objects of the same type, and they support operations like adding, removing, and iterating over elements.
   - `Map`: The `Map` interface, on the other hand, represents a mapping between keys and values, where each key is associated with a corresponding value. It is a data structure that allows you to retrieve a value by its key. Maps are used to implement dictionaries, associative arrays, or key-value pairs.

2. Unique Key-Value Association:
   - The key feature of a `Map` is its association of unique keys to values. Each key in a map must be unique, and the value associated with a key can be accessed using that key. This is different from collections, where each element can be independent, and there is no concept of a key-value association.

3. Methods and Behavior:
   - The methods defined in the `Collection` interface are focused on managing individual elements, such as `add`, `remove`, `contains`, and iteration methods like `forEach`, `iterator`, etc.
   - The methods defined in the `Map` interface are tailored to handle key-value pairs, such as `put`, `get`, `remove`, `containsKey`, `containsValue`, etc. These methods are used to manipulate the key-value associations in the map.

If the `Map` interface were to extend the `Collection` interface, it would imply that maps behave like collections, which is not the case. The distinction between collections and maps is fundamental and reflects their different use cases and underlying data structures. By keeping them separate, Java maintains a clear and consistent design, making it easier to work with both collections and maps in a more specialized and efficient way.
...
Compare-and-swap (CAS) is an atomic operation used in concurrent programming to achieve synchronization and mutual exclusion without the need for locks. It is a hardware-supported operation provided by modern processors and is widely used in programming languages, including Java, to implement thread-safe data structures and algorithms.

In Java, the `Atomic` classes in the `java.util.concurrent.atomic` package, such as `AtomicInteger`, `AtomicLong`, etc., provide CAS-based atomic operations. These classes are used to perform operations on variables in a way that ensures atomicity without explicit locking.

The CAS operation consists of three inputs:
1. A memory location (usually a variable or memory address).
2. The expected old value of the memory location.
3. The new value to be updated if the old value matches the expected value.

The CAS operation is performed atomically, meaning that it either succeeds, and the value is updated, or it fails, and the value remains unchanged. If the old value in memory matches the expected value, the new value is written to the memory location. Otherwise, the operation fails, and the caller can decide how to proceed, such as retrying the CAS operation.

In Java, the CAS operation is exposed through methods like `compareAndSet` in the `Atomic` classes. Here's a simplified example using `AtomicInteger` to demonstrate the concept:
...
public class CompareAndSwapExample {
    ...
    public static void main(String[] args) {
        ...
        // Perform compareAndSet operation
        ...
        if (success) {
            System.out.println("CAS operation successful: Value is updated to " + atomicInt.get());
        } else {
            System.out.println("CAS operation failed: Value remains unchanged.");
        }
    }
}
```

In this example, we have an `AtomicInteger` called `atomicInt` initialized with a value of `10`. We then attempt a CAS operation using `compareAndSet(expectedValue, newValue)`, where `expectedValue` is `10` and `newValue` is `20`. If the value in `atomicInt` is `10` (the expected value), the CAS operation will succeed, and the value will be updated to `20`. Otherwise, if the value is not `10`, the CAS operation will fail, and the value will remain unchanged.

CAS is particularly useful in scenarios where multiple threads are accessing shared data concurrently and need to perform updates safely without traditional locking mechanisms, which can lead to contention and performance issues. CAS allows for a non-blocking approach to concurrent programming, reducing contention and increasing scalability. However, it requires careful usage and understanding of potential race conditions and how to handle retries when the CAS operation fails.
